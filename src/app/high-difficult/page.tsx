...
const TOTAL_TIME = 10; // 各問題の制限時間（秒）
const NUM_QUESTIONS = 10; // 選ばれる問題の数
const NUM_CHOICES = 6; // 各問題の選択肢の数
const highSchools = [
  '茨城高専',
  '群馬高専',
  '木更津高専',
...
const Game: React.FC = () => {
  ...
  const saveScore = async () => {
    ...
    try {
      await axios.post('/api/saveScore-high', { name: playerName, score: score , school: selectedSchool });
      alert('スコアが保存されました！');
    } catch (error) {
